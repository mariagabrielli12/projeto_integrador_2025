<?php
/******************************
 * HARDENING DE SESSÃO E SEGURANÇA
 ******************************/
ini_set('session.cookie_httponly', 1); // impede JS de acessar o cookie
ini_set('session.use_strict_mode', 1); // força IDs de sessão válidos
if ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') || $_SERVER['SERVER_PORT'] == 443) {
    ini_set('session.cookie_secure', 1); // só envia cookie via HTTPS
}
...
header("Content-Security-Policy: default-src 'self'; img-src 'self' https://cdnjs.cloudflare.com; style-src 'self' https://cdnjs.cloudflare.com; script-src 'self' https://cdnjs.cloudflare.com;");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    // Sanitiza entrada (remove espaços e caracteres suspeitos)
    $matricula = trim($_POST['matricula']);
    $senha = $_POST['senha'];

    // Evita campos vazios ou muito longos
        ...
        if ($stmt = $conexao->prepare($sql)) {
            ...
            try {
                ...
                if ($result->num_rows === 1) {
                    ...
                    // ⚠️ RECOMENDADO: use password_hash() no cadastro e password_verify() aqui
                    // enquanto usa md5, deixo compatível:
                    if (md5($senha) === $usuario['senha_hash']) {

                        // --- LÓGICA DE 2FA A CADA 30 DIAS ---
                        if ($usuario['2fa_enabled'] == 0) {
                            // 1️⃣ NUNCA ATIVOU O 2FA → vai para ativação
                            $_SESSION['2fa_verifying_user_id'] = $usuario['id_usuario'];
                            header("Location: habilitar_2fa.php");
                            exit;
                        } else {
                            // 2️⃣ JÁ ATIVOU: checa validade (30 dias)
